...
def Key_Exists(Row, Key_List):
    for elem in Key_List:
        if elem.find(Row[:Row.find(',') - 1]) != -1:
            return True
    return False
...
def Insert_Into_Database(filename):
    ...
    cursor1 = connection.cursor()
    cursor1.execute("copy public.table1 FROM 'C:/PERSNL~1/CLIMAT~1/NCEI-D~1/Data_Edited/Table1/" + filename + "'  DELIMITER ',' NULL '-';")
    connection.close()
    '''with open("Data_Edited/Table2/" + filename):
        cursor2 = connection.cursor()
        cursor2.execute("copy public.table2 FROM 'C:/PERSNL~1/CLIMAT~1/NCEI-D~1/Data_Edited/Table2/" + filename + "'  DELIMITER ',' NULL '-';")
    '''

def Adjust_For_Table1(Row, g2):
    x = 0
    counter = 0
    while True:
        ...
        if counter == 7:
            break
    g2.write(Row[:y + 2] + Row[x + counter:])

def Adjust_For_Table2(Row, g3):
    x = 0
    counter = 0
    while True:
        if Row[x+counter:].find(',') != -1:
        ...
        if counter == 7:
            break
    g3.write(Row[:z + 1] + Row[y + 2: x + counter - 1] + '\n')
...
def Create_Attribute_Lists_From_Twin_Stations():  
    ...
    u = open("SQL/Results/double_keys_and_dates.csv", 'r')
    while True:
        a = u.readline()
        if not a:
            break
    ...
    while year <= end_year:
        ...
        while True:
            ...
            if Delete_Useless_Data(Row8) == False:
                g.write(Row8)
                Adjust_For_Table1(Row8, g2)
                if not Key_Exists(Row8, Key_List):
                    Adjust_For_Table2(Row8, g3)
            counter += 1
            counter2 += 1
            if counter2 % 1000 == 0:
                end = time.time()
